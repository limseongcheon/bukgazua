...
export async function getCaregiverRecommendations(request: CareRecommendationRequest): Promise<CaregiverRecommendationOutput> {
  ...
  // --- Step 1: Send email with care requirements ---
  // 이 부분은 Resend를 사용하여 관리자에게 이메일을 보냅니다.
  // 이메일 발송이 실패하더라도 사용자에게는 추천 결과가 정상적으로 보입니다.
  try {
    // .env.local 파일에서 Resend API 키를 읽어옵니다.
    ...
    if (!resendApiKey || resendApiKey.length < 5) {
        ...
        // .env.local 파일에서 설정된 관리자 수신 이메일 목록을 가져옵니다.
        const adminEmails = await getAdminEmails();

        if (adminEmails && adminEmails.length > 0) {
        // .env.local 파일에서 설정된 발신 전용 이메일 주소를 가져옵니다.
        const fromEmail = process.env.CARECONNECT_EMAIL_FROM || 'onboarding@resend.dev';
        
        console.log(`[Email Action] Attempting to send 'New Recommendation Request' email.`);
        ...
        const { data, error } = await resend.emails.send({
            ...
            <p><strong>희망 날짜:</strong> ${caregiverInput.requestedDates?.join(', ') || '미지정'}</p>
        ...
        if (error) {
            console.error("[Email Action] Resend API returned an error for 'New Recommendation Request':", error);
        } else {
            console.log('[Email Action] "New Recommendation Request" email sent successfully. Resend ID:', data?.id);
        }
        } else {
            console.warn('[Email Action] 관리자 수신 이메일이 .env.local 파일에 설정되지 않았습니다. 추천 요청 이메일을 보낼 수 없습니다.');
        }
    }
  } catch (error) {
    // This will catch constructor errors (e.g., missing API key) or other unexpected issues.
    console.error("[Email Action] A critical error occurred during the recommendation request email process (e.g., missing API key):", error);
  }

  // --- Step 2: Get recommendations and return to the user ---
  // 이 부분은 이메일 발송 여부와 관계없이 항상 실행됩니다.
...
export async function submitCaregiverInquiry(data: z.infer<typeof CaregiverInquirySchema>) {
  try {
    ...
    const fromEmail = process.env.CARECONNECT_EMAIL_FROM || 'onboarding@resend.dev';
    const { userName, userPhone, caregiverName } = data;
    ...
    const { data: responseData, error } = await resend.emails.send({
      ...
      html: `
        ...
        <hr />
        <p><strong>문의자 성명:</strong> ${userName}</p>
        <p><strong>문의자 연락처:</strong> ${userPhone}</p>
        <p><strong>선택한 간병인:</strong> ${caregiverName}</p>
        <hr />
